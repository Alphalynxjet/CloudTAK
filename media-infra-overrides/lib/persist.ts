    ...
    for (const path of paths.values()) {
        ...
        if (!exists) {
            ...
            if (
                ...
                // takserver.host override: re-apply when the SRT passphrase changes so
                // toggling/rotating Read/Write Security on an existing lease takes
                // effect (MediaMTX returns "" for an unset passphrase).
                || (exists.srtPublishPassphrase || '') !== (payload.srtPublishPassphrase || '')
                || (exists.srtReadPassphrase || '') !== (payload.srtReadPassphrase || '')
            ) {
